import type { ReactNode } from 'react'

function Glyph({ children }: { children: ReactNode }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="100%" height="100%" preserveAspectRatio="xMidYMid meet" viewBox="0 0 256 256">
      <g fill="currentColor">{children}</g>
    </svg>
  )
}

const ICONS: Record<string, ReactNode> = {
  'construction-loan-extra-month': (
    <Glyph>
      <path d="M192,168a40,40,0,0,1-40,40H128V128h24A40,40,0,0,1,192,168ZM112,48a40,40,0,0,0,0,80h16V48Z" opacity=".2" />
      <path d="M152,120H136V56h8a32,32,0,0,1,32,32,8,8,0,0,0,16,0,48.05,48.05,0,0,0-48-48h-8V24a8,8,0,0,0-16,0V40h-8a48,48,0,0,0,0,96h8v64H104a32,32,0,0,1-32-32,8,8,0,0,0-16,0,48.05,48.05,0,0,0,48,48h16v16a8,8,0,0,0,16,0V216h16a48,48,0,0,0,0-96Zm-40,0a32,32,0,0,1,0-64h8v64Zm40,80H136V136h16a32,32,0,0,1,0,64Z" />
    </Glyph>
  ),
  'digital-twin-pre-construction': (
    <Glyph>
      <path d="M128,129.09V232a8,8,0,0,1-3.84-1l-88-48.16a8,8,0,0,1-4.16-7V80.2a8,8,0,0,1,.7-3.27Z" opacity=".2" />
      <path d="M223.68,66.15,135.68,18h0a15.88,15.88,0,0,0-15.36,0l-88,48.17a16,16,0,0,0-8.32,14v95.64a16,16,0,0,0,8.32,14l88,48.17a15.88,15.88,0,0,0,15.36,0l88-48.17a16,16,0,0,0,8.32-14V80.18A16,16,0,0,0,223.68,66.15ZM128,32h0l80.34,44L128,120,47.66,76ZM40,90l80,43.78v85.79L40,175.82Zm96,129.57V133.82L216,90v85.78Z" />
    </Glyph>
  ),
  'follow-up-pre-construction-buyers': (
    <Glyph>
      <path d="M224,56l-96,88L32,56Z" opacity=".2" />
      <path d="M224,48H32a8,8,0,0,0-8,8V192a16,16,0,0,0,16,16H216a16,16,0,0,0,16-16V56A8,8,0,0,0,224,48ZM203.43,64,128,133.15,52.57,64ZM216,192H40V74.19l82.59,75.71a8,8,0,0,0,10.82,0L216,74.19V192Z" />
    </Glyph>
  ),
  'construction-loan-presale-requirement': (
    <Glyph>
      <path d="M95.8,56.2a28,28,0,1,1-39.6,0A28,28,0,0,1,95.8,56.2Zm104,104a28,28,0,1,0,0,39.6A28,28,0,0,0,199.8,160.2Z" opacity=".2" />
      <path d="M205.66,61.64l-144,144a8,8,0,0,1-11.32-11.32l144-144a8,8,0,0,1,11.32,11.31ZM50.54,101.44a36,36,0,0,1,50.92-50.91h0a36,36,0,0,1-50.92,50.91ZM56,76A20,20,0,1,0,90.14,61.84h0A20,20,0,0,0,56,76ZM216,180a36,36,0,1,1-10.54-25.46h0A35.76,35.76,0,0,1,216,180Zm-16,0a20,20,0,1,0-5.86,14.14A19.87,19.87,0,0,0,200,180Z" />
    </Glyph>
  ),
  'virtual-showroom-pre-construction': (
    <Glyph>
      <path d="M224,96v16a32,32,0,0,1-64,0V96H96v16a32,32,0,0,1-64,0V96L46.34,45.8A8,8,0,0,1,54,40H202a8,8,0,0,1,7.69,5.8Z" opacity=".2" />
      <path d="M231.69,93.81,217.35,43.6A16.07,16.07,0,0,0,202,32H54A16.07,16.07,0,0,0,38.65,43.6L24.31,93.81A7.94,7.94,0,0,0,24,96v16a40,40,0,0,0,16,32v72a8,8,0,0,0,8,8H208a8,8,0,0,0,8-8V144a40,40,0,0,0,16-32V96A7.94,7.94,0,0,0,231.69,93.81ZM54,48H202l11.42,40H42.61Zm98,56v8a24,24,0,0,1-48,0v-8ZM51.06,132.2A24,24,0,0,1,40,112v-8H88v8a24,24,0,0,1-35.12,21.26A7.88,7.88,0,0,0,51.06,132.2ZM200,208H56V151.2a40.57,40.57,0,0,0,8,.8,40,40,0,0,0,32-16,40,40,0,0,0,64,0,40,40,0,0,0,32,16,40.57,40.57,0,0,0,8-.8Zm16-96a24,24,0,0,1-11.07,20.2,8.08,8.08,0,0,0-1.8,1.05A24,24,0,0,1,168,112v-8h48Z" />
    </Glyph>
  ),
  'interactive-floor-plan-vs-pdf': (
    <Glyph>
      <path d="M160,72V216L96,184V40Z" opacity=".2" />
      <path d="M228.92,49.69a8,8,0,0,0-6.86-1.45L160.93,63.52,99.58,32.84a8,8,0,0,0-5.52-.6l-64,16A8,8,0,0,0,24,56V200a8,8,0,0,0,9.94,7.76l61.13-15.28,61.35,30.68A8.15,8.15,0,0,0,160,224a8,8,0,0,0,1.94-.24l64-16A8,8,0,0,0,232,200V56A8,8,0,0,0,228.92,49.69ZM104,52.94l48,24V203.06l-48-24ZM40,62.25l48-12v127.5l-48,12Zm176,131.5-48,12V78.25l48-12Z" />
    </Glyph>
  ),
  'sell-pre-construction-condos-faster': (
    <Glyph>
      <path d="M232,56v64L168,56Z" opacity=".2" />
      <path d="M232,48H168a8,8,0,0,0-5.66,13.66L188.69,88,136,140.69l-34.34-34.35a8,8,0,0,0-11.32,0l-72,72a8,8,0,0,0,11.32,11.32L96,123.31l34.34,34.35a8,8,0,0,0,11.32,0L200,99.31l26.34,26.35A8,8,0,0,0,240,120V56A8,8,0,0,0,232,48Zm-8,52.69L187.31,64H224Z" />
    </Glyph>
  ),
  'boutique-website-besides-renderings': (
    <Glyph>
      <path d="M216,40H40A16,16,0,0,0,24,56V88H232V56A16,16,0,0,0,216,40Z" opacity=".2" />
      <path d="M216,40H40A16,16,0,0,0,24,56V200a16,16,0,0,0,16,16H216a16,16,0,0,0,16-16V56A16,16,0,0,0,216,40ZM40,56H216V88H40Zm176,48v96H40V104Z" />
    </Glyph>
  ),
  'how-long-sell-out-20-40-unit-condo': (
    <Glyph>
      <path d="M200,75.64V40a8,8,0,0,0-8-8H64a8,8,0,0,0-8,8V76a16.07,16.07,0,0,0,6.4,12.8L114.67,128,62.4,167.2A16.07,16.07,0,0,0,56,180v36a8,8,0,0,0,8,8H192a8,8,0,0,0,8-8V180.36a16.09,16.09,0,0,0-6.35-12.77L141.26,128l52.39-39.59A16.09,16.09,0,0,0,200,75.64Z" opacity=".2" />
      <path d="M200,75.64V40a16,16,0,0,0-16-16H72A16,16,0,0,0,56,40V76a24.1,24.1,0,0,0,9.6,19.2L114.67,128,65.6,167.2A24.1,24.1,0,0,0,56,186.4V216a16,16,0,0,0,16,16H184a16,16,0,0,0,16-16V180.36a24.12,24.12,0,0,0-9.57-19.14L141.26,128l52.39-39.59A24.12,24.12,0,0,0,200,75.64ZM184,40V75.64a8.12,8.12,0,0,1-3.2,6.4L128,121.67,75.2,82A8.12,8.12,0,0,1,72,75.64V40Zm0,176H72V186.4a8.12,8.12,0,0,1,3.2-6.4l52.8-39.6,52.8,39.15a8.08,8.08,0,0,1,3.2,6.45Z" />
    </Glyph>
  ),
}

export default function InsightsTagIcon({ slug }: { slug: string }) {
  return ICONS[slug] ?? ICONS['construction-loan-extra-month']
}
